...
func Spectrogram(samples []float64, sampleRate int) ([][]complex128, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not downsample audio samples: %v", err)
	}

	numWindows := len(downsampledSamples) / (freqBinSize - hopSize)
...
func Downsample(input []float64, sourceSampleRate, targetSampleRate int) ([]float64, error) {
	...
	// combine some samples's average to a single sample
